...
const allData=JSON.stringify(data.map(l=>{const badDims=DIMS.filter(([k])=>l.dims[k]&&l.dims[k].verdict==='凶').map(([k])=>k);const em=badDims.length?badDims.map(k=>k+'→'+empower('interpret',k+'凶').transform).join('；'):'';return {y:l.year,ck:l.dk,d:Object.fromEntries(DIMS.map(([k])=>[k,l.dims[k].score])),em};}));
